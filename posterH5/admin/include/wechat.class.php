...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    protected function setCache($name,$value,$expired){
        $expired += time();
        return file_put_contents(WX_CACHE_DIR.$name.'.cache',json_encode(array('val'=>$value,'expTime'=>$expired)));
    }
    ...
    protected function getCache($name){
        $name = WX_CACHE_DIR.$name.'.cache';
        if(is_file($name))
        {
            $data = json_decode(file_get_contents($name),true);
            if($data['expTime']<time() && $data['expTime']!=0)return false;
            return $data['val'];
        }
     ...
     * @return raw data
     */
    public function getMedia($media_id,$is_video=false,$repeat=true){
        if (!$this->access_token && !$this->checkAuth()) return false;
        //原先的上传多媒体文件接口使用 self::UPLOAD_MEDIA_URL 前缀
        ...
        if ($result)
        {
            if (is_string($result)) {
                ...
                if (isset($json['errcode'])) {
                    $this->errCode = $json['errcode'];
                    $this->errMsg = $json['errmsg'];
                    if($this->errCode == 40001 && $repeat)
                    {
                        $this->resetAuth();
                        return $this->getMedia($media_id,$is_video,false);
                    }
                    return false;
                }
            }
     ...
     * @return boolean|array
     */
    public function sendCustomMessage($data,$repeat=true){
        if (!$this->access_token && !$this->checkAuth()) return false;
        ...
        if ($result)
        {
            ...
            if (!$json || !empty($json['errcode'])) {
                ...
                {
                    $this->resetAuth();
                    return $this->sendCustomMessage($data,false);
                }
                return false;
            }
            return $json;
